...
The \pkg{mefa} package provides standardized environment and a and convenient tool for ecologists and biogeographers working on biodiversity data sets in \proglang{R}. The object classes (`stcs' and `mefa') and \proglang{S}3 methods provide a coherent framework for data preprocessing of structured data sets. Based on `mefa' objects, reports can be generated in plain text or \LaTeX~format. It was presented how the objects can be directly used in further analyses for a variety of ecological problems.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
/home/psolymos/Projects/mefa/jss
