...
Compared to previous versions (< 2.0, \citep{mefa-comecol-08}), the package has been extensively rewritten to enhance efficiency and speed, and with a focus on methods, but the data model remained the same. A stable version of the package is available under the terms of the General Public License from the Comprehensive \proglang{R} Archive Network (CRAN, \url{http://cran.r-project.org/package=mefa}), developmental version is available at the \proglang{R}-Forge (\url{http://mefa.r-forge.r-project.org/}).
...
\begin{figure}[!htb]
    \begin{center}
        \includegraphics[width=0.6\textwidth]{mefa-jss-mefastr}
    \end{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section[S3 method]{\proglang{S}3 methods}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The \pkg{mefa} package provides standardized environment and a and convenient tool for ecologists and biogeographers working on biodiversity data sets in \proglang{R}. The object classes (`stcs' and `mefa') and \proglang{S}3 methods provide a coherent framework for data preprocessing of structured data sets. Based on `mefa' objects, reports can be generated in plain text or \LaTeX~format. It is presented how the objects can be directly used in further analyses for a variety of ecological problems.
