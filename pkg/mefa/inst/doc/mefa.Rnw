...
This vignette is based on a manuscript submitted to the \emph{Journal of Statistical Software}, processed with \pkg{mefa}
\Sexpr{packageDescription("mefa", field="Version")} in \Sexpr{R.version.string} on \today.
}
...
\begin{figure}[!htb]
    \begin{center}
        \includegraphics[width=0.9\textwidth]{mefa-mefastr}
    \end{center}
