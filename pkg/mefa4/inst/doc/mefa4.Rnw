...
a \code{"Mefa"} object cannot have 3 dimensions, only 2. This might not be
...
The equivalent processing with \pkg{mefa4}:
<<>>=
b4 <- abmibirds
...
The aggregation has also improved quite a bit:
<<>>=
...
The redesign of the old S3 classes onto S4 ones resulted in large savings in computing time
