...
\noindent
The document provide some insights on the general data model, which can be used effectively inbiotic data handling. Than, using a real world example, it gives an overview of S3 object classes and methods provided by the \texttt{mefa} package. Reporting options and possible ways of further data analysis are also briefly discussed. This vignette can be used as reerence for the \texttt{dolina} demo of the package.
...
Local \texttt{ZIP} file and source is avaliable at \href{http://cran.r-project.org/src/contrib/Descriptions/mefa.html}{http://cran.r-project.org/\-src/contrib/\-Descriptions/\-mefa.html}.
...
\texttt{svn checkout svn://\-svn.r-forge.r-project.org/\-svnroot/mefa}, or by following the link: \href{https://r-forge.r-project.org/plugins/scmsvn/viewcvs.php/?root=mefa}{https://\-r-forge.r-project.org/\-plugins/\-scmsvn/\-viewcvs.php/\-?root=mefa}.
...
What are \texttt{<NA>}-s? This data frame contains sample and taxonomic information in shortened way, sample and species identifiers are shown only for the first cases. This "notebook-style" data can be filled with respective data by the function \texttt{sscount} (see also helper function \texttt{fill.count}):
...
In this \texttt{sscount} object, rows represent the basic units (lots) containing sample, species and segment identifiers (treated as characters) and the count. First three columns are treated as factors, and segment may contain \texttt{<NA>} if corresponding row contains species \texttt{zc} (because, it has no sense to use anysegment name for a pseudo-species). Count is 1 in this case. Basically, count should be integer, but when the result of an experiment is not an integer vaue (eg.~biomass, as a measurement variable), numeric values can be used by specifying the \texttt{digits} argument in the \texttt{sscount} function (see manual for further references).

\texttt{ssc\$data} contains the data taken from the filled data frame of \texttt{dol.count}.
Cross-tabulation of an \texttt{sscount} object results in an \texttt{xcount} object. Crosstabulation can be done for all or any segments, listed by
...
Plot method is available for \texttt{xcount} objects (Fihure~\ref{xchist}), plotting histograms by default:
...
plot(xc.all, type="rank")
@

\begin{figure}[tb]
	\includegraphics[width=1\textwidth]{mefa-vignette-xcrank}
	\caption{Plotting method for \texttt{xcount} objects showing species abundance curves.\label{xcrank}}
\end{figure}
...
And an object of \texttt{xorder} for species attributes:
...
or using taxonomic or trophic groups of species:
...
In the resulting mefa object, respective parts will be excluded for both the crosstabulation and attribute teble. Empty rows or columns can be removed by specifying the \texttt{empty} argument (see manual). In the end of this subsection, let's have look into the internal structure of a \texttt{mefa} object:
...
<<>>=
library(mefa)
report.mefa("dolina-report.tex", mf, ordering = "species", 
    biotic.data = c(1:5), 
...
The cross-tabulated count data stored in \texttt{xcount} and \texttt{mefa} objects can be used readily to calculate sample specific results (abundance, richness, etc.) or can be used in multivariate analyses. These possibilities of further data analyses are illustrated in the demo script (\texttt{demo(dolina)}). (Will be included here later \ldots)

\section{How to cite \texttt{mefa}?}

Please refer to the actual answer returned by the command (\texttt{citation("mefa")}).
